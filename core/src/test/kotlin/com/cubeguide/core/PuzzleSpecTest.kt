...
class PuzzleSpecTest {
 @Test fun registryContainsExactlyTheSixRegularCubeSizes() {
  assertEquals(6,PuzzleRegistry.all.size)
  assertEquals(6,PuzzleRegistry.all.map { it.id }.toSet().size)
 ...
 @Test fun everyPuzzleDefinesACompleteScanAndMoveContract() {
  ...
  assertEquals((2..7).toSet(),PuzzleRegistry.all.mapNotNull { it.squareSize }.toSet())
 }
