 ...
 DisposableEffect(lifecycle) {
  ...
  future.addListener({
   if(!disposed) try {
    ...
    val squareDetector=FaceDetector(gridSize)
    var lastFrame=0L
    analysis!!.setAnalyzer(executor) { image ->
     try {
      ...
      if(now-lastFrame>=120) {
       ...
       val detection=try { squareDetector.detect(rotated).copy(aspectRatio=rotated.width.toFloat()/rotated.height) } finally { if(rotated!==original) rotated.recycle(); original.recycle() }
