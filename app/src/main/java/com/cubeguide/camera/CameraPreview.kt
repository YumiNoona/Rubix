...
import com.cubeguide.core.ScanShape
import org.opencv.android.OpenCVLoader
import java.util.concurrent.Executors

@Composable fun CameraPreview(modifier: Modifier,onDetection: (Detection)->Unit,torch: Boolean=false,onFlashAvailable: (Boolean)->Unit={},gridSize:Int=3,scanShape:ScanShape=ScanShape.SQUARE_GRID) {
 ...
 DisposableEffect(lifecycle) {
  ...
  future.addListener({
   if(!disposed) try {
    ...
    val squareDetector=if(scanShape==ScanShape.SQUARE_GRID) FaceDetector(gridSize) else null
    val triangleDetector=if(scanShape==ScanShape.TRIANGLE_GRID) TriangleFaceDetector() else null
    require(squareDetector!=null || triangleDetector!=null) { "Camera detector for $scanShape is not implemented." }
    var lastFrame=0L
    analysis!!.setAnalyzer(executor) { image ->
     try {
      ...
      if(now-lastFrame>=120) {
       ...
       val detection=try { (squareDetector?.detect(rotated) ?: triangleDetector!!.detect(rotated)).copy(aspectRatio=rotated.width.toFloat()/rotated.height) } finally { if(rotated!==original) rotated.recycle(); original.recycle() }
