...
private data class VQuad(val points: List<VP>,val color: Color)
@Composable internal fun VirtualCubeView(cube: VirtualCube,modifier: Modifier=Modifier) {
 ...
 Canvas(modifier.semantics { contentDescription="Interactive ${cube.size} by ${cube.size} virtual cube. Drag to rotate." }.pointerInput(Unit) { detectDragGestures { change,drag -> change.consume();yaw+=drag.x*.008f;pitch=(pitch+drag.y*.008f).coerceIn(-1.2f,1.2f) } }) {
  ...
  VirtualCube.geometry(cube.size).forEachIndexed { index,g ->
   ...
   val half=.82f
   quads+=VQuad(listOf(center+right*-half+down*-half,center+right*half+down*-half,center+right*half+down*half,center+right*-half+down*half).map(::camera),Color(preferences.color(cube.stickers[index])))
  }
  ...
  quads.filter { q -> val a=q.points[1]+q.points[0]*-1f;val b=q.points[3]+q.points[0]*-1f;a.cross(b).dot(VP(0f,0f,12f)+q.points[0]*-1f)<0 }.sortedBy { it.points.map { p->p.z }.average() }.forEach { quad ->
   ...
   drawPath(path,Color(0xFF061014),style=Stroke(max(2f,5f/cube.size)))
  }
 }
}
