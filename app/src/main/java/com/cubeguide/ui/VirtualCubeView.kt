...
@Composable internal fun VirtualCubeView(cube: VirtualCube,modifier: Modifier=Modifier,move:Move?=null,replay:Int=0,onAnimationProgress:(Float)->Unit={}) {
 val preferences=LocalAppPreferences.current
 var yaw by remember { mutableFloatStateOf(-0.62f) };var pitch by remember { mutableFloatStateOf(0.52f) }
 val pulse=remember { Animatable(1f) }
 val turn=remember(cube.stickers,move) { Animatable(0f) };val progressCallback by rememberUpdatedState(onAnimationProgress)
 LaunchedEffect(cube.stickers,move) { if(move==null) { pulse.snapTo(0.965f);pulse.animateTo(1f,spring(stiffness=Spring.StiffnessMediumLow,dampingRatio=0.72f)) } else pulse.snapTo(1f) }
 LaunchedEffect(turn) { snapshotFlow { turn.value }.collect { progressCallback(it) } }
 LaunchedEffect(turn,replay) { if(move!=null) { if(turn.value>0f) turn.snapTo(0f);turn.animateTo(1f,tween(preferences.animationMillis*(if(move.turns==2) 2 else 1))) } }
 Canvas(modifier.semantics { contentDescription="Interactive ${cube.size} by ${cube.size} virtual cube. Drag to rotate." }.pointerInput(Unit) { detectDragGestures { change,drag -> change.consume();yaw+=drag.x*.008f;pitch=(pitch+drag.y*.008f).coerceIn(-1.2f,1.2f) } }) {
  ...
  val turnAxis=move?.let { Geometry.normals[it.face.ordinal].vp() };val turnAngle=if(move==null) 0f else -turn.value*(if(move.turns==3)-1 else move.turns)*PI.toFloat()/2
  fun layer(point:VP,position:VP):VP { if(turnAxis==null || position.dot(turnAxis)<edge-.01f) return point;return point*cos(turnAngle)+turnAxis.cross(point)*sin(turnAngle)+turnAxis*(turnAxis.dot(point)*(1-cos(turnAngle))) }
  ...
  Face.entries.forEach { face ->
   ...
   quads+=VQuad(listOf(center+right*-span+down*-span,center+right*span+down*-span,center+right*span+down*span,center+right*-span+down*span).map { camera(layer(it,normal*edge)) },camera(layer(normal,normal*edge)),Color(0xFF05090B),false)
  }
  VirtualCube.geometry(cube.size).forEachIndexed { index,g ->
   ...
   val half=.82f
   quads+=VQuad(listOf(center+right*-half+down*-half,center+right*half+down*-half,center+right*half+down*half,center+right*-half+down*half).map { camera(layer(it,pos)) },camera(layer(normal,pos)),Color(preferences.color(cube.stickers[index])))
  }
